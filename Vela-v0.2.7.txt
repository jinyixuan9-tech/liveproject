          ...
          async mount(container, roche) {
            ...
            const refreshHomeFeedContent = async () => {
              ...
              candidates.forEach((channel, index) => {
                ...
                const ageMs = (20 + Math.floor(Math.random() * (36 * 60))) * 60 * 1000 + index * 7 * 60 * 1000;
                const at = Date.now() - ageMs;
                const mins = Math.round(ageMs / 60000);
                const time = mins < 60 ? `${mins} 分钟前` : mins < 1440 ? `${Math.floor(mins/60)} 小时前` : `${Math.floor(mins/1440)} 天前`;
                const id = `home-feed-${String(channel.id)}-${at.toString(36)}`;
                const previous = state.homeFeedByChannel[String(channel.id)];
                if (previous && !state.communityPosts.some(p => String(p.id) === String(previous.id))) state.communityPosts.push({ ...previous, ownerType:"channel", ownerId:String(channel.id) });
                state.homeFeedByChannel[String(channel.id)] = { id, ownerType:"channel", ownerId:String(channel.id), text:pickOne(pool)||pool[0], translation:"", image:"", time, likes:Math.max(0,Math.round((Number(channel.followers||0)||120)*(0.02+Math.random()*0.08))), accessMode:identitiesFollowingChannel(channel.id,"paid").length ? (Math.random()<0.58?"public":(Math.random()<0.68?"free":"paid")) : (Math.random()<0.65?"public":"free"), at };
              });
