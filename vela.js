...
观众弹幕应有差异，参考频道粉丝画像，不要所有网友都像同一个人。频道、角色和平台账号应有自己的活动，不要永远围绕 user 运转。商业邀约、私信与推荐内容要符合频道规模和内容方向，避免高频、重复或无缘由出现。`;
  ...
  function createDefaultState() {
    return {
      ...
      dmThreads: {
        "biz-mellow": [
          { id:"biz-intro", kind:"text", sender:"other", name:"Mellow Studio", avatar:"M", text:"你好，我们是 Mellow Studio。最近在做一组偏安静居家氛围的香薰企划，觉得你的直播风格很合适，想邀请你看看这份合作。", at:Date.now()-180000 },
          { id:"biz-card-demo", kind:"business", direction:"incoming", status:"pending", brandName:"Mellow Studio", productName:"Cloud Mist 室内香薰", title:"Cloud Mist · 夜间香气直播合作", content:"希望在一次自然聊天直播中体验香薰，并结合房间、睡前或日常放松场景介绍香型感受；不要求硬性念稿。", dateText:"9/12 · 20:00（可协商）", hostLabel:"我的直播间", cashAmountCNY:3200, adCopy:"把一天慢慢收起来，让房间先安静下来。Cloud Mist 是偏轻柔的木质与白茶香，不抢镜，更适合陪着夜晚待一会儿。", adLink:"https://example.com/cloud-mist", at:Date.now()-120000 }
        ]
      },
...
/* v0.2.9 · align live-chat translate/delete controls; keep recommendation live behavior pending next pass */
...
.vela-roche .v-msg-swipe{position:relative;overflow:hidden;border-bottom:1px solid var(--v-line)}.vela-roche .v-msg-swipe:last-child{border-bottom:0}.vela-roche .v-msg-swipe .v-msg{width:100%;transition:transform .18s ease;border-bottom:0!important}.vela-roche .v-msg-swipe .v-msg-delete{position:absolute;right:0;top:0;bottom:0;width:72px;transform:translateX(100%);transition:transform .18s ease}.vela-roche .v-msg-swipe.is-swipe-open .v-msg{transform:translateX(-72px)}.vela-roche .v-msg-swipe.is-swipe-open .v-msg-delete{transform:none}
      ...
      apps: [
        {
          ...
          async mount(container, roche) {
            ...
            state.messages = safeArray(state.messages).filter(msg => !["aster-dm","char-aster","char-noir"].includes(String(msg?.id||"")) && !["Aster","Noir"].includes(String(msg?.name||"")));
            ...
            if (!state.messages.some(msg => String(msg?.id || "") === "biz-mellow")) {
              state.messages.unshift({ id:"biz-mellow", avatar:"M", name:"Mellow Studio", handle:"@mellow_studio", badge:"商务联系", preview:"你好，我们是 Mellow Studio，主营生活方式与香氛产品。想先和你聊聊一款新品的直播合作，你这边会接广告吗？", unread:1 });
            }
            if (!state.recommendationSamplesSeeded) {
              const previewSamples = [
                { id:"rec-preview-ko", external:true, locale:"ko", name:"하린", handle:"@harin.room", avatar:"하", title:"오늘은 늦게까지 같이 있어요", titleTranslation:"今天一起待到晚一点。", category:"Seoul · Late night chat", categoryTranslation:"首尔 · 深夜聊天", viewers:1842, live:true, startedAt:Date.now()-22*60*1000, at:Date.now()-22*60*1000, chatSeed:[
                  { user:"minji.zip", avatar:"민", text:"오늘 목소리 완전 편하다 ㅋㅋ", translation:"今天的声音听起来特别让人放松哈哈。" },
                  { user:"doyeon_17", avatar:"도", text:"주말에도 이 시간쯤 켤 거예요?", translation:"周末也会差不多这个时间开播吗？" },
                  { user:"Aki", avatar:"A", text:"韓国語わからないけど雰囲気好き", translation:"虽然听不懂韩语，但很喜欢这个氛围。" }
                ] },
                { id:"rec-preview-ja", external:true, locale:"ja", name:"凛", handle:"@rin_after5", avatar:"凛", title:"作業しながら少しだけ", titleTranslation:"一边做事一边播一会儿。", category:"Tokyo · 雑談", categoryTranslation:"东京 · 闲聊", viewers:926, live:true, startedAt:Date.now()-37*60*1000, at:Date.now()-37*60*1000, chatSeed:[
                  { user:"mio_25", avatar:"み", text:"今日のBGMめっちゃ好き", translation:"今天的背景音乐特别喜欢。" },
                  { user:"haru", avatar:"は", text:"仕事終わって今来た〜", translation:"刚下班，现在才进来～" },
                  { user:"soo_kr", avatar:"수", text:"분위기 좋다, 그냥 켜두고 듣는 중", translation:"氛围真好，我就开着当背景听。" }
                ] }
              ];
              previewSamples.forEach(sample => { if (!state.recommendedLives.some(item => String(item?.id || "") === sample.id)) state.recommendedLives.unshift(sample); });
            ...
            if (!oldMellowThread.some(item => item?.kind === "business")) {
              oldMellowThread.push({ id:"biz-card-demo-v026", kind:"business", direction:"incoming", status:"pending", brandName:"Mellow Studio", productName:"Cloud Mist 室内香薰", title:"Cloud Mist · 夜间香气直播合作", content:"希望在一次自然聊天直播中体验香薰，并结合房间、睡前或日常放松场景介绍香型感受；不要求硬性念稿。", dateText:"9/12 · 20:00（可协商）", hostLabel:"我的直播间", cashAmountCNY:3200, adCopy:"把一天慢慢收起来，让房间先安静下来。Cloud Mist 是偏轻柔的木质与白茶香，不抢镜，更适合陪着夜晚待一会儿。", adLink:"https://example.com/cloud-mist", at:Date.now()-120000 });
            }
            ...
            const buildRefreshLiveCard = (channel, index = 0) => {
              const locale = channel ? inferChannelLocale(channel) : pickOne(["ko","ko","ja","en"]) || "ko";
              const pack = liveLocalePack(locale);
              const at = Date.now() - (6 + Math.floor(Math.random() * 44)) * 60 * 1000;
              const externalNames = locale === "ko"
                ? [{name:"서윤",handle:"@seoyun.log",avatar:"서"},{name:"민아",handle:"@mina.room",avatar:"민"}]
                : locale === "ja"
                  ? [{name:"凛",handle:"@rin_after5",avatar:"凛"},{name:"美緒",handle:"@mio_note",avatar:"美"}]
                  : [{name:"June",handle:"@june_room",avatar:"J"},{name:"Mika",handle:"@mika_here",avatar:"M"}];
              const external = pickOne(externalNames) || externalNames[0];
              const title = pickOne(pack.titles) || pack.titles[0];
              const titleTranslation = locale === "ko" ? ({
                "오늘은 그냥 같이 얘기해요":"今天就随便一起聊聊天吧。",
                "조금 늦게까지 있을게요":"今天会待到稍微晚一点。",
                "작업하면서 천천히 얘기 중":"一边做事一边慢慢聊天。"
              }[title] || "正在轻松聊天。") : locale === "ja" ? ({
                "少しだけ雑談します":"稍微闲聊一会儿。",
                "作業しながらゆっくり":"一边做事一边慢慢播。",
                "今日は静かめに配信":"今天会安静一点地直播。"
              }[title] || "正在闲聊。") : locale === "en" ? ({
                "just hanging out for a bit":"就随便待一会儿。",
                "late night catch-up":"深夜聊聊天。",
                "working and chatting":"一边做事一边聊天。"
              }[title] || "正在轻松直播。") : "";
              return {
                id:`rec-${channel?.id || "global"}-${Date.now().toString(36)}-${index}-${Math.random().toString(36).slice(2,5)}`,
                external:!channel, locale, name:channel?.name || external.name, handle:channel?.handle || external.handle, avatar:channel?.avatar || external.avatar,
                title, titleTranslation, category:channel?.contentStyle || pack.category, categoryTranslation:channel?.contentStyle ? "" : pack.translation,
                viewers:Math.max(12, Math.round((Number(channel?.followers || 0) || (500 + Math.random()*2400)) * (0.02 + Math.random()*0.06))),
                ownerId:channel?.id || "", live:true, startedAt:at, at, chatSeed:pack.chat.map(item => ({ ...item }))
              };
            };
...
候选频道：${JSON.stringify(summaries)}` }]);
                let items = normalizeHomeAIItems(extractJSON(aiText), candidates);
                if (!items.length) {
                  const shuffled = [...candidates].sort(() => Math.random()-.5);
                  const count = Math.max(1, Math.min(shuffled.length, 1 + Math.floor(Math.random()*2)));
                  items = shuffled.slice(0,count).map((channel,index) => {
                    const doLive = !channel.live && scheduleAllowsLiveNow(channel) && Math.random() < .34;
                    const locale = inferChannelLocale(channel);
                    const pack = liveLocalePack(locale);
                    const ageMs = (30 + Math.floor(Math.random() * 60 * 50)) * 60 * 1000;
                    return doLive
                      ? { channelId:channel.id, type:"live", publishedAt:new Date(Date.now()-Math.min(ageMs,90*60*1000)).toISOString(), title:pickOne(pack.titles)||pack.titles[0], titleTranslation:locale === "zh" ? "" : "正在轻松直播。", category:channel.contentStyle || pack.category, categoryTranslation:"", chatSeed:pack.chat }
                      : { channelId:channel.id, type:"post", publishedAt:new Date(Date.now()-ageMs).toISOString(), text:pickOne(["今天随手更新一下。","刚把手头的事情收完，来这里留一点记录。","最近在慢慢调整自己的节奏。"]), translation:"" };
                  });
                }
                let added = 0;
                items.forEach((item,index) => {
                  ...
                  if (String(item.type) === "live") {
                    ...
                    channel.chatSeed = safeArray(item.chatSeed).map(x => ({ user:String(x?.user || "viewer"), avatar:initials(x?.user || "V"), text:String(x?.text || ""), translation:String(x?.translation || "") })).filter(x => x.text);
                    added += 1;
                    return;
                  }
                  const id = `home-ai-${String(channel.id)}-${at.toString(36)}-${index}-${Math.random().toString(36).slice(2,5)}`;
                  state.communityPosts.push({ id, ownerType:"channel", ownerId:String(channel.id), text:String(item.text || "").trim() || "更新一下。", translation:String(item.translation || ""), image:"", time:formatRelativeAt(at), likes:Math.max(0,Math.round((Number(channel.followers||0)||120)*(.02+Math.random()*.08))), accessMode:"public", ageRequirement:"none", at, generatedByHome:true });
            ...
            const refreshRecommendedLives = async () => {
              ...
              try {
                ...
                const pool = [...candidates].sort(() => Math.random()-.5);
                const batch = Array.from({length:3}, (_,index) => buildRefreshLiveCard(pool[index] || null, index));
            ...
            const renderMessages = () => {
              ...
              host.innerHTML = `<div class="v-card">${state.messages.map(msg => {
                ...
                return `<div class="v-msg-swipe" data-dm-row="${escapeHTML(msg.id)}"><button class="v-msg" data-action="open-message" data-message-id="${escapeHTML(msg.id)}"><div class="v-mini">${avatarHTML(msg.avatar, msg.name)}</div><div class="v-meta"><div class="v-name">${escapeHTML(msg.name)}<span class="v-relation ${rel.cls}">${escapeHTML(rel.text)}</span></div><div class="v-preview">${escapeHTML(msg.preview || "暂无消息")}</div></div>${msg.unread ? `<span class="v-unread">${msg.unread}</span>` : ""}</button><button class="v-msg-delete" data-action="delete-dm-conversation" data-message-id="${escapeHTML(msg.id)}">删除</button></div>`;
              }).join("")}</div>`;
            };
            ...
            const openLive = (live) => {
              ...
              const viewerComposer = `<div class="v-composer">${liveIdentityPicker}<input data-role="live-input" placeholder="回车发送弹幕…" maxlength="120"><button data-action="gift-demo" aria-label="送礼物" title="送礼物">🎁</button><button data-action="summon-live" aria-label="召唤 / 继续直播" title="召唤 / 继续直播">${planeIconHTML()}</button></div>`;
            ...
            const advanceLive = async ({ fromUser = false, userText = "" } = {}) => {
              ...
              const chatPools = {
                ko:[
                  { user:"yoo_n", avatar:"Y", text:"아까 그 얘기 너무 웃겼어 ㅋㅋ", translation:"刚才那个话题真的太好笑了哈哈。" },
                  { user:"minji.zip", avatar:"민", text:"오늘 평소보다 좀 조용하네", translation:"今天比平时安静一点。" },
                  { user:"doyeon_17", avatar:"도", text:"지금 분위기 너무 편하다", translation:"现在这个氛围特别舒服。" },
                ...
                ja:[
                  { user:"mio_25", avatar:"み", text:"今の話もう少し聞きたい", translation:"还想再听一点刚才那个话题。" },
                  { user:"haru", avatar:"は", text:"今日いつもより静かだね", translation:"今天比平时安静一点呢。" },
                  { user:"rinrin", avatar:"り", text:"この時間の配信ほんと助かる", translation:"这个时间开播真的太好了。" },
                  { user:"soo_kr", avatar:"수", text:"분위기 좋다 그냥 듣는 중", translation:"氛围真好，我就这样听着。" }
                ],
                en:[
                  { user:"Mika", avatar:"M", text:"keep going, this is nice", translation:"继续播吧，这样很好。" },
                  { user:"June", avatar:"J", text:"just got here, what did I miss?", translation:"刚进来，我错过什么了？" },
                  { user:"Noah", avatar:"N", text:"the vibe tonight is really calm", translation:"今晚的氛围真的很安静。" },
                  { user:"yuna", avatar:"유", text:"오늘 분위기 좋다", translation:"今天的氛围真好。" }
                ],
                zh:[
                  { user:"小羽", avatar:"羽", text:"刚才那个话题还想再听一点。", translation:"" },
                  { user:"momo", avatar:"M", text:"今天感觉比平时安静一点哈哈。", translation:"" },
                  { user:"Aki", avatar:"A", text:"这个时间开播刚刚好。", translation:"" },
                  { user:"Mika", avatar:"M", text:"keep going, this is nice", translation:"继续播吧，这样很好。" }
                ]
              };
              const chatPool = chatPools[lang] || chatPools.zh;
            ...
            const summonPostReplies = async (postId) => {
              ...
              const locale = ownerChannel ? inferChannelLocale(ownerChannel) : "zh";
              const pools = {
                ko:[
                  { user:"luna_08", avatar:"L", text:"와 이 분위기 진짜 좋다", translation:"哇，这个氛围真的很好。" },
                  { user:"익명", avatar:"익", text:"이런 느낌의 글 더 올려줘요", translation:"多发一点这种感觉的动态吧。" },
                  { user:"minji.zip", avatar:"민", text:"오늘 사진 색감 완전 좋네", translation:"今天照片的色调特别好。" },
                  { user:"momo", avatar:"M", text:"次の投稿も楽しみ", translation:"也很期待下一条动态。" }
                ],
                ja:[
                  { user:"momo", avatar:"M", text:"この雰囲気すごく好き", translation:"特别喜欢这个氛围。" },
                  { user:"haru", avatar:"は", text:"次の投稿も楽しみにしてる", translation:"也很期待下一条动态。" },
                  { user:"mio_25", avatar:"み", text:"写真の感じ今日すごくいい", translation:"今天照片的感觉特别好。" },
                  { user:"yuna", avatar:"유", text:"분위기 너무 좋다", translation:"氛围真的很好。" }
                ],
                en:[
                  { user:"cloudy", avatar:"C", text:"this feels so cozy honestly", translation:"说真的，这种感觉特别舒服。" },
                  { user:"June", avatar:"J", text:"love this kind of update", translation:"很喜欢这种类型的更新。" },
                  { user:"Mika", avatar:"M", text:"the vibe in this post is so nice", translation:"这条动态的氛围真好。" },
                  { user:"momo", avatar:"M", text:"次の投稿も楽しみ", translation:"也很期待下一条动态。" }
                ],
                zh:[
                  { user:"小羽", avatar:"羽", text:"这个氛围真的很好。", translation:"" },
                  { user:"momo", avatar:"M", text:"下一条也想看这种感觉的。", translation:"" },
                  { user:"Aki", avatar:"A", text:"这张的色调很好看。", translation:"" },
                  { user:"luna_08", avatar:"L", text:"와 이 분위기 진짜 좋다", translation:"哇，这个氛围真的很好。" }
                ]
              };
              const pool = pools[locale] || pools.zh;
              const current = safeArray(state.postReplies[id]);
              ...
              while (additions.length < 5) {
                const i = additions.length;
                additions.push({ ...pool[(offset + i) % pool.length], id:`reply-${Date.now().toString(36)}-${i}-${Math.random().toString(36).slice(2,5)}`, isUser:false });
              }
              if (additions.length > 5) additions.length = 5;
            ...
            const openMessage = (id) => {
              let msg = state.messages.find(x => String(x.id) === String(id));
              if (!msg && String(id) === "biz-mellow") {
                msg = { id:"biz-mellow", avatar:"M", name:"Mellow Studio", handle:"@mellow_studio", badge:"商务联系", preview:"你好，我们是 Mellow Studio，主营生活方式与香氛产品。想先和你聊聊一款新品的直播合作，你这边会接广告吗？", unread:1 };
            ...
            const summonDirectMessage = async (messageId) => {
              ...
              const system = `${VELA_DEFAULT_PRESET}\n\n你正在模拟 Vela 私信中 USER 之外的对方。绝对禁止替 USER 发言、行动、做决定。Enter 只发送 USER 自己写的内容，小飞机召唤时即使 USER 没有新消息，对方也可以自然继续说自己的事，不需要强行围着 USER。对方身份：${msg.name || "对方"} ${channel?.handle || msg.handle || ""}。${persona ? `角色设定：${persona}` : ""}
...
如果群主自然想参与，可以偶尔输出 {"role":"owner","text":"群主消息","translation":""}；不要每轮都让群主出现。成员风格要有差异，不要每轮所有人都说话。`;
            ...
            root.addEventListener("pointerdown", event => {
              ...
              dmSwipe = { row, x:Number(event.clientX || 0), y:Number(event.clientY || 0), moved:false };
            });
            root.addEventListener("pointermove", event => {
              if (!dmSwipe) return;
              ...
              dmSwipe.moved = true;
              if (dx < -18) dmSwipe.row.classList.add("is-swipe-open");
              if (dx > 18) dmSwipe.row.classList.remove("is-swipe-open");
            });
            root.addEventListener("pointerup", () => {
              if (dmSwipe?.moved) suppressClickUntil = Date.now() + 260;
              dmSwipe = null;
            });
