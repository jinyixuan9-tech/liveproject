...
/* v0.2.5 · follow/subscription split + compact identity switch */
      ...
      apps: [
        {
          ...
          async mount(container, roche) {
            ...
            const openProfile = (ownerType, id) => {
              ...
              const viewerIdentity = state.identities.find(item => String(item.id) === String(state.viewerIdentityId)) || state.identities[0];
              const viewerFollows = ownerType === "channel" && viewerIdentity ? isFollowing("identity", viewerIdentity.id, "channel", entity.id) : false;
              const channelFollowsViewer = ownerType === "channel" && viewerIdentity ? isFollowing("channel", entity.id, "identity", viewerIdentity.id) : false;
              const followLabel = viewerFollows ? (channelFollowsViewer ? "互关" : "已关注") : (channelFollowsViewer ? "回关" : "关注");
              const followButton = ownerType === "channel" && viewerIdentity ? `<button class="v-profile-follow ${viewerFollows ? "is-on" : ""}" data-action="toggle-profile-follow" data-channel-id="${escapeHTML(entity.id)}">${escapeHTML(followLabel)}</button>` : "";
              const followNote = ownerType === "channel" && channelFollowsViewer && !viewerFollows ? `<div class="v-profile-follow-note">关注了你</div>` : "";
              const otherFollowedIdentities = ownerType === "channel" ? state.identities.filter(identity => String(identity.id) !== String(viewerIdentity?.id || "") && isFollowing("channel", entity.id, "identity", identity.id)) : [];
              const otherFollowNote = otherFollowedIdentities.length ? `<div class="v-profile-followed-ids"><span>还关注了你的</span>${otherFollowedIdentities.slice(0,3).map(identity => `<span class="v-profile-followed-mini" title="${escapeHTML(identity.displayName || identity.handle || "我的账号")}">${avatarHTML(identity.avatar || "", identity.displayName || "U")}</span>`).join("")}</div>` : "";
            ...
            const openSubscriptionSettings = (ownerType, ownerId) => {
              ...
              openScreen("subscription-settings", `<header class="v-subhead"><button data-action="close-screen" data-screen-name="subscription-settings">‹</button><div class="v-meta"><strong>订阅设置</strong><div class="v-hint">${escapeHTML(profileName(entity, ownerType))}</div></div><button class="v-head-action" data-action="save-subscription-settings" data-owner-type="${escapeHTML(ownerType)}" data-owner-id="${escapeHTML(ownerId)}">保存</button></header><div class="v-subbody"><div class="v-subscription-editor-grid"><section class="v-subscription-editor-tier"><h3>免费订阅</h3><div class="v-editform"><div class="v-fieldpair"><div class="v-field"><label>粉丝符号 / Emoji</label><input data-sub-field="freeEmoji" maxlength="4" value="${escapeHTML(cfg.free.emoji || "🌊")}"></div><div class="v-field"><label>方案名称</label><input data-sub-field="freeName" maxlength="24" value="${escapeHTML(cfg.free.name || "支持者")}"></div></div><div class="v-field"><label>卡片封面 URL</label><input data-sub-field="freeCover" value="${escapeHTML(cfg.free.cover || "")}" placeholder="https://..."></div><div class="v-field"><label>或上传卡片封面</label><input type="file" accept="image/*" data-sub-cover-file="free"></div><div class="v-subscription-locked"><b>固定简介</b><p>关注频道动态，解锁免费订阅贴文与直播推送。</p><b>固定权益</b><p>获得粉丝身份。</p></div></div></section><section class="v-subscription-editor-tier is-paid"><h3>付费订阅</h3><div class="v-editform"><div class="v-fieldpair"><div class="v-field"><label>粉丝符号 / Emoji</label><input data-sub-field="paidEmoji" maxlength="4" value="${escapeHTML(cfg.paid.emoji || "💎")}"></div><div class="v-field"><label>方案名称</label><input data-sub-field="paidName" maxlength="24" value="${escapeHTML(cfg.paid.name || "会员")}"></div></div><div class="v-field"><label>月费标准 · Vela Coin</label><input data-sub-field="paidCoinPrice" type="number" min="1" max="999999" step="1" value="${Number(cfg.paid.coinPrice || 500)}"></div><div class="v-field"><label>卡片封面 URL</label><input data-sub-field="paidCover" value="${escapeHTML(cfg.paid.cover || "")}" placeholder="https://..."></div><div class="v-field"><label>或上传卡片封面</label><input type="file" accept="image/*" data-sub-cover-file="paid"></div><div class="v-field"><label>付费订阅社群群名</label><input data-sub-field="paidCommunity" maxlength="40" value="${escapeHTML(cfg.paid.communityName || `${profileName(entity, ownerType)} · 会员社群`)}"></div><div class="v-subscription-locked"><b>固定简介</b><p>支持喜欢的主播，解锁付费订阅内容与社群。</p><b>固定权益</b><p>获得付费粉丝身份<br>加入订阅社群<br>获得主播第一手资讯</p></div></div></section></div></div>`);
            };
            ...
            const saveSubscriptionSettings = async (ownerType, ownerId) => {
              ...
              state.subscriptionProfiles[subscriptionProfileKey(ownerType, ownerId)] = {
                free: {
                  ...current.free,
                  emoji: read("freeEmoji") || "🌊",
                  name: read("freeName") || "支持者",
                  cover: read("freeCover"),
                  intro: "关注频道动态，解锁免费订阅贴文与直播推送。",
                  benefits: ["获得粉丝身份"]
                },
                paid: {
                  ...current.paid,
                  emoji: read("paidEmoji") || "💎",
                  name: read("paidName") || "会员",
                  coinPrice: Math.max(1, Math.min(999999, Math.round(Number(read("paidCoinPrice") || 500)))),
                  cover: read("paidCover"),
                  intro: "支持喜欢的主播，解锁付费订阅内容与社群。",
                  benefits: ["获得付费粉丝身份", "加入订阅社群", "获得主播第一手资讯"],
                  communityName: read("paidCommunity") || `${profileName(entity, ownerType)} · 会员社群`
                }
              };
              const paidCommunityName = state.subscriptionProfiles[subscriptionProfileKey(ownerType, ownerId)]?.paid?.communityName || `${profileName(entity, ownerType)} · 会员社群`;
              const paidCommunityKey = communityChatKey(ownerType, ownerId, "paid");
              if (state.communitySettings?.[paidCommunityKey]) state.communitySettings[paidCommunityKey] = { ...state.communitySettings[paidCommunityKey], name: paidCommunityName };
              await persist();
