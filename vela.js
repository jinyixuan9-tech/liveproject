...
/* v0.2.10 · DM tap/open hardening; legacy placeholder DM cleanup; AI-only refreshed contacts */
      ...
      apps: [
        {
          ...
          async mount(container, roche) {
            ...
            // Do not seed a fake Mellow conversation on clean installs. If an older install already has it,
            // keep the existing thread so the user can still inspect/migrate that conversation.
            ...
            if (legacyMellowMsg || legacyMellowRows.length) {
              if (legacyMellowMsg) {
                if (String(legacyMellowMsg.preview || "").includes("Cloud Mist 香氛机合作邀约")) legacyMellowMsg.preview = "你好，我们是 Mellow Studio，主营生活方式与香氛产品。想先和你聊聊一款新品的直播合作，你这边会接广告吗？";
                if (String(legacyMellowMsg.badge || "") === "合作邀约") legacyMellowMsg.badge = "商务联系";
              }
              ...
              if (!oldMellowThread.length && legacyMellowMsg?.preview) {
                oldMellowThread.push({ id:"biz-intro", kind:"text", sender:"other", name:legacyMellowMsg.name || "Mellow Studio", avatar:legacyMellowMsg.avatar || "M", text:String(legacyMellowMsg.preview), at:Date.now()-180000 });
              }
              state.dmThreads["biz-mellow"] = oldMellowThread;
            } else {
              delete state.dmThreads["biz-mellow"];
            }
            ...
            let toastTimer = null;
            const generationBusy = { home:false, recommended:false };
            ...
            const setRefreshBusy = (kind, busy) => {
              ...
              const action = kind === "home" ? "refresh-home" : "refresh-recommended";
...
返回严格 JSON 数组：[{"kind":"stranger|fan|business","name":"公开昵称或品牌名","handle":"@handle","avatar":"简短字符","badge":"陌生私信|粉丝|商务联系","locale":"ko|ja|en|zh","preview":"对方第一条自然私信；品牌先问合作意向，不要一上来发正式合同","businessProfile":"仅 business 可写公司主营/品牌风格/产品方向，否则空"}]。语言按账号地区自然分布，陌生人不要都说中文。`;
              ...
              if (!Array.isArray(data)) { toast("DM 生成失败，请再试一次"); return; }
              ...
              if (!batch.length) { toast("DM 生成失败，请再试一次"); return; }
              state.messages = [...batch, ...state.messages].slice(0, 50);
              await persist(); renderMessages(); toast(`新增 ${batch.length} 条私信`);
            };
