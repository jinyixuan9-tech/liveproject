...
/* v0.2.11 · keep one Mellow business demo; DM refresh busy feedback; refresh remains AI-driven */
      ...
      apps: [
        {
          ...
          async mount(container, roche) {
            ...
            if (legacyMellowMsg || legacyMellowRows.length) {
              ...
              const oldMellowThread = legacyMellowRows.filter(item => item && typeof item === "object" && !String(item?.id || "").startsWith("biz-card-demo"));
              if (!oldMellowThread.length && legacyMellowMsg?.preview) {
                oldMellowThread.push({ id:"biz-intro", kind:"text", sender:"other", name:"Mellow Studio", avatar:"M", text:String(legacyMellowMsg.preview), translation:"", at:Date.now()-180000 });
              }
            ...
            const ensureMellowBusinessDemo = () => {
              ...
              if (!msg) {
                msg = {
                  id, avatar:"M", name:"Mellow Studio", handle:"@mellow_studio", badge:"商务联系",
                  preview, unread:1, external:true, strangerKind:"business", locale:"zh",
                  businessProfile:"生活方式与香氛品牌；产品方向为居家香氛、空间气味与桌面生活方式。"
                };
                state.messages.unshift(msg);
              } else {
                msg.avatar = msg.avatar || "M";
              ...
              state.dmThreads[id] = safeArray(state.dmThreads[id]).filter(item => item && typeof item === "object" && !String(item?.id || "").startsWith("biz-card-demo"));
              if (!state.dmThreads[id].length) {
                state.dmThreads[id].push({ id:"biz-intro", kind:"text", sender:"other", name:"Mellow Studio", avatar:"M", text:preview, translation:"", at:Date.now()-180000 });
              }
              return msg;
            };
            const inferChannelLocale = channel => {
              ...
              if (/台湾|臺灣|香港|中文|台北|Taipei|Hong Kong/i.test(source)) return "zh";
            ...
            const liveLocalePack = locale => {
              const packs = {
                ko: {
                  ...
                  chat:[
                    { user:"yuna.zip", avatar:"유", text:"오늘 분위기 진짜 좋다", translation:"今天这个氛围真的很好。" },
                    { user:"mii_02", avatar:"미", text:"방금 왔는데 무슨 얘기 중이었어요?", translation:"刚进来，刚才在聊什么呀？" },
                    { user:"sora", avatar:"소", text:"주말에도 방송 켜줘요 ㅠ", translation:"周末也开播吧呜呜。" }
                  ]
                },
                ja: {
                  ...
                  chat:[
                    { user:"mio", avatar:"み", text:"今日の雰囲気すごく好き", translation:"今天的氛围特别喜欢。" },
                    { user:"haru_5", avatar:"は", text:"今来た〜何してたの？", translation:"刚进来～刚才在做什么？" },
                    { user:"rinrin", avatar:"り", text:"この時間の配信ありがたい", translation:"这个时间开播太好了。" }
                  ]
                },
                en: {
                  ...
                  chat:[
                    { user:"mika", avatar:"M", text:"this is such a nice background stream", translation:"这场直播特别适合当背景听。" },
                    { user:"noah", avatar:"N", text:"just got here, what did I miss?", translation:"刚进来，我错过什么了？" },
                    { user:"jules", avatar:"J", text:"the vibe tonight is so calm", translation:"今晚的氛围好安静。" }
                  ]
                },
                zh: {
                  ...
                  chat:[
                    { user:"momo", avatar:"M", text:"刚进来，今天好安静哈哈", translation:"" },
                    { user:"小羽", avatar:"羽", text:"这个时间开播刚刚好", translation:"" },
                    { user:"aki", avatar:"A", text:"背景音好舒服", translation:"" }
                  ]
                }
              };
            ...
            const normalizeRecommendedAIItems = (data, count, allowedAliases = []) => {
              ...
              for (const raw of arr) {
                ...
                const locale = ["ko","ja","en","zh"].includes(String(raw.locale || "").toLowerCase()) ? String(raw.locale).toLowerCase() : detectLiveLanguage({ title, category });
                ...
                const chatSeed = safeArray(raw.chatSeed).slice(0, 6).map((item, index) => ({
                  ...
                  avatar:String(item?.avatar || initials(item?.user || "V")),
                ...
                out.push({
                  ...
                  name,
                  handle,
                  avatar:String(alias?.avatar || raw.avatar || initials(name)),
                  ownerId:alias?.id || "",
...
[{"channelId":"必须从候选 channelId 中选","type":"post|live","publishedAt":"ISO时间","text":"贴文正文，live 可空","translation":"正文非中文时给中文翻译，否则空","title":"直播标题，post 可空","titleTranslation":"标题非中文时给中文翻译，否则空","category":"直播内容/分类","categoryTranslation":"非中文时给中文翻译，否则空","chatSeed":[{"user":"观众网名","text":"原语言弹幕","translation":"中文翻译或空"}]}]
...
候选频道：${JSON.stringify(summaries)}` }]);
                ...
                items.forEach((item,index) => {
                  ...
                  if (String(item.type) === "live") {
                    ...
                    channel.chatSeed = safeArray(item.chatSeed).map(x => ({ user:String(x?.user || "").trim(), avatar:initials(x?.user || "V"), text:String(x?.text || "").trim(), translation:String(x?.translation || "") })).filter(x => x.user && x.text);
                    added += 1;
                    return;
                  }
...
陌生主播要像真实平台用户：姓名/网名、handle、地区语言、直播标题、分类、当前动作和已存在聊天室都要彼此一致，不要使用固定模板或重复老名字。返回严格 JSON 数组，恰好 ${count} 条。每条字段：
[{"source":"stranger|alias","ownerId":"alias 时填候选 id，否则空","name":"公开昵称","handle":"@handle","avatar":"可用昵称首字或简短字符","locale":"ko|ja|en|zh","creatorProfile":"一句主播风格/地区提示","title":"原语言直播标题","titleTranslation":"非中文则中文翻译，否则空","category":"原语言直播内容/分类","categoryTranslation":"非中文则中文翻译，否则空","viewers":1234,"startedMinutesAgo":20,"openingAction":"镜头里正在发生的动作","openingSpeech":"主播正在说的话，符合 locale","openingTranslation":"openingSpeech 非中文时给中文翻译，否则空","chatSeed":[{"user":"观众网名","avatar":"简短字符","text":"原语言弹幕","translation":"中文翻译或空"}]}]
...
允许的角色副号候选：${JSON.stringify(aliasSummary)}` }]);
                ...
                await persist(); renderRecommended(); toast(`新增 ${batch.length} 个推荐直播`);
              } finally {
...
返回严格 JSON 数组：[{"kind":"stranger|fan|business","name":"公开昵称或品牌名","handle":"@handle","avatar":"简短字符","badge":"陌生私信|粉丝|商务联系","locale":"ko|ja|en|zh","preview":"对方第一条自然私信；品牌先问合作意向，不要一上来发正式合同","businessProfile":"仅 business 可写公司主营/品牌风格/产品方向，否则空"}]。语言按账号地区自然分布，陌生人不要都说中文。`;
                const aiText = await runVelaAI([{ role:"system", content:system }, { role:"user", content:`当前时间：${new Date().toString()}` }]);
                ...
                for (const raw of data) {
                  ...
                  const msg = { id, avatar:String(raw.avatar || initials(name)), name, handle, badge, preview, unread:1, external:true, strangerKind:kind, locale:String(raw.locale || ""), businessProfile:String(raw.businessProfile || "") };
                  batch.push(msg);
                  state.dmThreads[id] = [{ id:`seed-${id}`, kind:"text", sender:"other", name, avatar:msg.avatar, text:preview, translation:"", at:Date.now() }];
                }
...
返回严格 JSON 对象：{"events":[{"actorId":"必须来自允许主体 id","actorName":"显示名","action":"自然动作，可空","speech":"实际说的话，可空；使用主要语言","translation":"speech 非中文时给中文翻译，否则空"}],"chat":[{"user":"各不相同的观众网名","avatar":"简短字符","text":"原语言弹幕","translation":"中文翻译或空","tipCoin":0,"giftCoin":0}]}。
要求：events 0~${Math.max(1, Math.min(3, allowedActors.length))} 条，chat 约 ${expectedChat} 条；不要复用固定网名或固定动作模板；giftCoin/tipCoin 只能偶尔出现，数值应自然。`;
              ...
              const chat = safeArray(data.chat).slice(0, 4).map((item,index) => ({
                user:String(item?.user || "").trim(), avatar:String(item?.avatar || initials(item?.user || "V")), text:String(item?.text || "").trim(), translation:String(item?.translation || "").trim(), tipCoin:Math.max(0, Math.round(Number(item?.tipCoin || 0))), giftCoin:Math.max(0, Math.round(Number(item?.giftCoin || 0))), at:Date.now()+index
              })).filter(item => item.user && item.text);
...
返回严格 JSON 数组，恰好 5 条：[{"role":"viewer|owner","user":"viewer 时填自然网名；owner 可空","text":"评论原语言","translation":"非中文时给中文翻译，否则空","replyTo":"可空；需要回复已有评论时写对方公开名"}]。评论者网名和语言要有差异，不要复用固定模板；owner 只能是贴文作者本人，且不必每批出现。绝对不能替 USER 发评论。`;
              ...
              for (const item of data) {
                ...
                if (!user) continue;
                additions.push({ id:`reply-${Date.now().toString(36)}-${additions.length}-${Math.random().toString(36).slice(2,5)}`, user, avatar:ownerReply ? String(post.avatar || initials(post.name || "V")) : initials(user), text, translation:String(item.translation || "").trim(), replyTo:String(item.replyTo || "").trim(), isUser:false });
              }
            ...
            const endCurrentLive = async () => {
              ...
              if (userEligible) {
                const panel = q('[data-screen="live"] [data-role="live-economy-panel"]');
                if (panel) {
                  panel.innerHTML = `<div class="v-live-summary-card"><h3>本场直播已结束</h3><div class="v-hint">${escapeHTML(live.title || "直播")}</div><div class="v-live-summary-grid"><div class="v-live-summary-box"><small>本场打赏</small><b>🔷${tipCoin.toLocaleString("zh-CN")}</b></div><div class="v-live-summary-box"><small>本场礼物</small><b>🔷${giftCoin.toLocaleString("zh-CN")}</b></div><div class="v-live-summary-box"><small>${businessTailPaid ? "商务尾款" : "商务合作"}</small><b>${escapeHTML(formatCash(businessTailPaid || businessCashCNY))}</b></div><div class="v-live-summary-box"><small>${collab ? "你的 50% 分成" : "你的结算"}</small><b>${escapeHTML(formatCoin(userCoin))}${userCashCNY ? ` · ${escapeHTML(formatCash(userCashCNY))}` : ""}</b></div></div><div class="v-publish-note" style="margin-top:12px">打赏和礼物进入 Vela Coin 余额；普通手动广告收入直接进入可提现余额；商务合同接受时结定金，完成约定直播后结尾款。角色分成只写入角色后台收益记录。</div><button class="v-live-summary-close" data-action="close-live-summary-card">完成</button></div>`;
                  panel.classList.add("is-open");
                } else {
                  closeScreen("live");
                }
            ...
            const savePlatformSettings = async () => {
              ...
              state.platformSettings = { ...(state.platformSettings || {}), recommendationCount, allowRoleAliasesInRecommendations, allowBusinessDM, activityLevel, strangerInteraction };
            ...
            const renderChatBubble = (item, { group = false, threadKey = "", threadType = "dm" } = {}) => {
              ...
              const dmTranslation = threadType === "dm" && item.translation
                ? `<div class="v-dm-bubble-translation" data-translation-id="${escapeHTML(trId)}" hidden>${escapeHTML(item.translation)}</div>`
                : "";
              const bubbleAction = threadType === "dm" && item.translation ? ` data-action="toggle-dm-bubble-translation" data-translation-target="${escapeHTML(trId)}"` : "";
              const regularTranslation = threadType !== "dm" && item.translation ? translationHTML(trId, item.translation) : "";
              return `<div class="v-chat-row ${mine ? "is-me" : ""}"${longpress}>${mine ? "" : `<div class="v-social-avatar">${avatarHTML(item.avatar || "", item.name || "V")}</div>`}<div class="v-chat-stack">${title}<div class="v-chat-bubble"${bubbleAction}>${escapeHTML(item.text || "")}${dmTranslation}${regularTranslation}</div></div></div>`;
            };
...
{"type":"business","brandName":"公司或品牌","productName":"产品","productPriceCNY":299,"title":"合作标题","content":"直播要求与交付内容","totalFeeCNY":3200,"depositCNY":800,"balanceCNY":2400,"scheduledAt":"YYYY-MM-DDTHH:mm","endsAt":"YYYY-MM-DDTHH:mm","host":"user|counterpart","adCopy":"广告词","adLink":"链接","text":"可选附言","translation":""}
...
如果对方自然同意 USER 已经发出的待确认卡，可以在普通 message JSON 里把 acceptCardId 写成卡片 ID：${pendingOutgoing?.id || "无待确认卡"}。不要自动替 USER 接受任何东西。`;
              ...
              if (data.type === "live_invite" || data.type === "business") {
                const incoming = {
                  ...
                  productPriceCNY: data.type === "business" ? Math.max(0, Number(data.productPriceCNY || 0)) : 0,
                  totalFeeCNY: data.type === "business" ? Math.max(0, Number(data.totalFeeCNY ?? data.cashAmountCNY ?? 0)) : 0,
                  cashAmountCNY: data.type === "business" ? Math.max(0, Number(data.totalFeeCNY ?? data.cashAmountCNY ?? 0)) : 0,
                  depositCNY: data.type === "business" && data.depositCNY != null ? Math.max(0, Number(data.depositCNY || 0)) : undefined,
                  balanceCNY: data.type === "business" && data.balanceCNY != null ? Math.max(0, Number(data.balanceCNY || 0)) : undefined,
                  deliverables: String(data.content || ""), adCopy: String(data.adCopy || ""), adLink: String(data.adLink || ""), mode: data.mode === "same-room" ? "same-room" : "online", includesLive:true,
                  scheduledAt: data.scheduledAt && !Number.isNaN(new Date(data.scheduledAt).getTime()) ? new Date(data.scheduledAt).getTime() : 0,
                  endsAt: data.endsAt && !Number.isNaN(new Date(data.endsAt).getTime()) ? new Date(data.endsAt).getTime() : 0,
                  dateText: "",
                  at: Date.now()
                };
                if (incoming.kind === "business") {
                  if (incoming.scheduledAt && (!incoming.endsAt || incoming.endsAt <= incoming.scheduledAt)) incoming.endsAt = incoming.scheduledAt + 2 * 3600000;
                  const amounts = businessAmounts(incoming); incoming.totalFeeCNY = amounts.total; incoming.cashAmountCNY = amounts.total; incoming.depositCNY = amounts.deposit; incoming.balanceCNY = amounts.balance; incoming.dateText = formatBusinessWindow(incoming.scheduledAt, incoming.endsAt);
                } else incoming.dateText = String(data.dateText || (incoming.scheduledAt ? formatDateTextFromInput(data.scheduledAt) : "时间待定"));
                ...
                if (!replyText) { button?.classList.remove("is-loading"); toast("对方这次没有生成有效消息"); return; }
                state.dmThreads[messageId].push({ id: `dm-${Date.now().toString(36)}`, kind: "text", sender: "other", name: msg.name || "对方", avatar: msg.avatar || "", text: replyText, translation: String(data.translation || ""), at: Date.now() });
              }
              state.dmThreads[messageId] = state.dmThreads[messageId].slice(-240);
...
如果群主自然想参与，可以偶尔输出 {"role":"owner","text":"群主消息","translation":""}；不要每轮都让群主出现。成员风格要有差异，不要每轮所有人都说话。`;
            ...
            const clearVelaPluginData = async () => {
              ...
              fresh.platformSettings = { recommendationCount: 3, allowBusinessDM: true, activityLevel: 50, strangerInteraction: "normal", allowRoleAliasesInRecommendations: false };
              state = fresh;
              await roche?.storage?.set?.("vela-shell-state", {});
              await persist();
              mountMarkup(root, state);
